...
def get_duration(
    ...
    if path.strip().endswith("|"):
        ...
        # exception type from kaldi_native_io ? (std::runtime_error via pybind11)
        except:
...
def load_kaldi_data_dir(
        ...
        Recording(
            ...
            sources=[
                AudioSource(
                    ...
                    source=(
                        path_or_cmd[:-
                                    1] if path_or_cmd.endswith("|") else path_or_cmd
                    ),
                )
            ],
            sampling_rate=sampling_rate,
            num_samples=compute_num_samples(
                durations[recording_id], sampling_rate),
    ...
    if segments.is_file():
        ...
        with segments.open() as f:
            supervision_segments = [sup_string.strip().split()
                                    for sup_string in f]
        ...
        with segments_path.open() as segments_f, feats_path.open() as feats_f:
            for segments_line, feats_line in zip(segments_f, feats_f):
                ...
                if segment_id != utt_id:
                    raise ValueError(
                        f"{segments_path} and {feats_path} not aligned.")
